...
public class StampServiceBean {
    ...
    public long putTree(StampTreeModel model) {
        ...
        try {
            StampTreeModel exist = (StampTreeModel) em.createQuery(QUERY_TREE_BY_USER_PK)
    ...
    public String syncTree(StampTreeModel model) {
        ...
        if (vesion >= 0) {
            // 保存
            //- debug
//            model.setLastUpdated(new java.util.Date());
//            Logger.getLogger("open.dolphin").info("################################################");
//            Logger.getLogger("open.dolphin").log(Level.INFO, "debug treeName -> {0}", model.getName());
//            Logger.getLogger("open.dolphin").log(Level.INFO, "debug userID   -> {0}", model.getUserModel().getId());
//            Logger.getLogger("open.dolphin").log(Level.INFO, "debug byte     -> {0}", model.getTreeBytes().toString());
//            Logger.getLogger("open.dolphin").log(Level.INFO, "debug ID       -> {0}", model.getId());
//            Logger.getLogger("open.dolphin").log(Level.INFO, "debug DATE     -> {0}", model.getLastUpdated());
//            Logger.getLogger("open.dolphin").log(Level.INFO, "debug RegDATE  -> {0}", model.getUserModel().getRegisteredDate());
//            Logger.getLogger("open.dolphin").info("################################################");
    ...
    public StampTreeHolder getTrees(long userPK) {
        ...
        // パーソナルツリーを取得する
        //-----------------------------------
        List<StampTreeModel> list = (List<StampTreeModel>) em.createQuery(QUERY_TREE_BY_USER_PK)
    ...
    public int unsubscribeTrees(List<Long> list) {
        ...
        for (int i = 0; i < len; i += 2) {
            ...
            List<SubscribedTreeModel> removes = (List<SubscribedTreeModel>) em.createQuery(QUERY_SUBSCRIBED_BY_USER_PK_TREE_ID)
                    .setParameter(USER_PK, userPK)
