...
public class PVTServiceBean {
    ...
    private static final String QUERY_PVT_ALL = "from PatientVisitModel";
//masuda^    
    ...
    public int addPvt(PatientVisitModel pvt) {
        ...
        try {
            ...
            PatientModel exist = (PatientModel) em.createQuery(QUERY_PATIENT_BY_FID_PID)
            ...
            // getSingleResult -> catch しなくても throw されるはず...
            sb = new StringBuilder();
            sb.append("患者ID重複[").append(fid).append(":").append(patient.getPatientId()).append("]");
            Logger.getLogger("open.dolphin").info(sb.toString());
            throw new RuntimeException("患者ID重複エラー");
        } catch (Exception e) {
            throw new RuntimeException("患者登録アプリケーションエラー");
        }
//minagawa$
        // ここからPVT登録処理
...
//minagawa$
        ...
        // カルテの PK を得る
        long karteId = (Long) em.createQuery(QUERY_KARTE_ID_BY_PATIENT_ID)
                .setParameter(ID, pvt.getPatientModel().getId())
                .getSingleResult();
        // 予約を検索する
        Logger.getLogger("open.dolphin").log(Level.INFO, "KarteID {0}", karteId);
        Logger.getLogger("open.dolphin").log(Level.INFO, "Time {0}", contextHolder.getToday().getTime());
        List<AppointmentModel> c = null;
        try {
            c = em.createQuery(QUERY_APPO_BY_KARTE_ID_DATE)
                    .setParameter(ID, karteId)
                    .setParameter(DATE, contextHolder.getToday().getTime())
                    .getResultList();
        } catch (NoResultException e) {
            Logger.getLogger("open.dolphin").log(Level.INFO, "Appointment result 0.");
        }
