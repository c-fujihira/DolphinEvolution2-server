...
//masuda^
            // 最終文書日
            try {
                Date lastDocDate = (Date) em.createQuery(QUERY_LASTDOC_DATE)
                        .setParameter(KARTE_ID, karteId)
                        .getSingleResult();
...
//    }
    ...
    public List<SchemaModel> getS3Images(String fid, int firstResult, int maxResult) {
        ...
        List<SchemaModel> ret = (List<SchemaModel>) em.createQuery(QUERY_SCHEMA_BY_FACILITY_ID)
                .setParameter(FID, fid + "%")
...
//minagawa$        
    ...
    public LetterModel getLetter(long letterPk) {
        ...
        LetterModel ret = (LetterModel) em.createQuery(QUERY_LETTER_BY_ID)
    ...
    public LetterModel getLetterReply(long letterPk) {
        ...
        LetterModel ret = (LetterModel) em.createQuery(QUERY_REPLY_BY_ID)
                .setParameter(ID, letterPk)
                .getSingleResult();
        return ret;
    }
