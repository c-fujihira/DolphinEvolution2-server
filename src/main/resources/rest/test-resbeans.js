...
function TestSupport() {
    this.wadlDoc = null;
    this.wadlURL = '';
    this.wadlErr = 'WADLにアクセスできません。RESTfulアプリケーションを再起動し、このページをリフレッシュしてください。';
    ...
    this.viewIds = [
        ...
        { "id" : "raw" , "name":"rawデータのビュー", "type":"rawContent"}, 
        { "id" : "structure" , "name":"サブリソース", "type":"structureInfo"},
        { "id" : "header" , "name":"ヘッダー", "type":"headerInfo"},
        { "id" : "monitor" , "name":"HTTPモニター", "type":"monitorContent"}];
...
TestSupport.prototype = {    
    ...
    initFromWadl : function () {
        ...
        if(wDoc != null) {
            ...
            this.updatepage('content', '<span class=bld>ヘルプ・ページ</span><br/><br/><p>WADLにアクセスできません。RESTアプリケーションを再起動し、このページをリフレッシュしてください。</p><p>このエラーがまだ表示され、FirebugプラグインをインストールしたFirefoxからこのページにアクセスしている場合は、<br/>ローカル・ファイルに対してFirebugを無効にする必要があります。無効にするには、Firefoxのメニュー・バーから<br/>「ツール」>「Firebug」>「Disable Firebug for Local Files」を選択します。</p>');
        }            
    },
    ...
    doShowDynamicResource : function (uri, mName, mediaType) {
        ...
        str += "</select></span></td>";
        str += "<td width=30/>"
        str += "<td><span id=j_id14><a class='Btn2_sun4 Btn1Hov_sun4' onclick='ts.addParam()'>パラメータの追加</a>";
    ...
    clearAll : function() {
        this.clearOutput();
        this.updatepage('request', 'ナビゲーション・バー(このページの左側)上でテストするノードを選択します。');
    ...
    monitor : function (method, xmlHttpReq, param) {
        ...
        var s = 'リクエスト: ' + this.currentMethod + ' ' + cURL + '\n<br/>' + params +
                    '\n\n<br/><br/>ステータス: ' + xmlHttpReq.status + ' (' + xmlHttpReq.statusText + ')'+
                    '\n\n<br/><br/>タイム・スタンプ: ' + ' ' + xmlHttpReq.getResponseHeader('Date') + '';
        var prevs = '';
    ...
    updateContent : function (content) {
        var showRaw = true;
        var cErr = 'コンテンツにコンテナ-被コンテナ関係がない可能性があります。コンテンツについては、rawデータのビューを参照してください。';
    ...
    createTabs : function(tableContent, rawContent, showRaw) {
        ...
        if(subResources == null)
            subResources = '使用可能なサブリソースがありません。';
        var tabs = '<br/><span class=bld>ステータス:</span> '+ this.currentXmlHttpReq.status+' ('+this.currentXmlHttpReq.statusText+')<br/><br/>'+
                    '<span class=bld>レスポンス:</span> '+
    ...
    getTab : function (id, actived) {
        ...
        if(actived)
            return '<td class="Tab1TblSelTd_sun4"><div title="現在の選択: テキスト・フィールド" class="Tab1SelTxtNew_sun4"><a name="selectedTabAnchor" id="tab'+id+'"/>'+name+'</div></td>';
        else {
            if(id == '')
    ...
    debug : function(message) {
        ...
        if(dbgComp == null) {
            ...
            dbgComp.innerHTML = '<table><tr><td><span style="color: blue">Restデバッグ・ウィンドウ</span></td><td>'+addActionStr + '</td></tr></table><br/>';
        }
    ...
    getHttpRequest : function() {
        ...
        {    // Internet Explorer 6.0+, 5.0+
            ...
            catch (e)
            {
                ...
                {
                    ts.debug("ブラウザでAJAXがサポートされていません。");
                }
            }
        }
        return xmlHttpReq;
        ...
        try {
            ...
            if (this.isResponseReady(method, xmlHttpReq, content, monitor)) {
              ...
              if(rtext == undefined || rtext == '' || rtext.indexOf('HTTP Status') != -1) {
                  var err = method+' リクエストFailed RequestFailed --> ステータス: (' 
                  	   + xmlHttpReq.status+')\n<br/>'+
                      'レスポンス: {<br/>' + xmlHttpReq.responseText + "<br/>}";
